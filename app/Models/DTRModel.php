...
class DTRModel extends Model
{
    ...
    public function getAccumulatedHours()
    {
        ...
        $builder = $db->table($this->table);
        $builder->select("interns.name as name, interns.start_date as start_date, CAST(SUM(TIME_TO_SEC(TIMEDIFF(time_out, time_in))) / 3600 AS DECIMAL(10,2)) AS hours, count(dtr.time_out) as dtr_count, interns.req_hrs as required_hours");
        $builder->join('interns', 'dtr.user_id = interns.id');
