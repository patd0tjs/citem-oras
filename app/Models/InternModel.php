...
class InternModel extends Model
{
    ...
    public function getLedgerDetails()
    {
        ...
        $builder->select("interns.id as id, interns.name as name, schools.name as school, interns.start_date as start_date, 
                            interns.req_hrs as total_hours, ROUND(SUM(TIME_TO_SEC(TIMEDIFF(dtr.time_out, dtr.time_in))) / 3600, 2) AS rendered_hours, 
                            department, is_active, link");
